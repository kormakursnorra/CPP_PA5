...
We've developed a terminal-based maze game in C++ where the player has to navigate a randomly generated, perfect maze to reach the exit before the allotted time is up. Before stepping into the maze, the player must choose a difficulty level (Easy, Medium, Hard, No Escape) that determines the size of the maze and the time limit given to him. Throughout the duration of the game, "breadcrumbs" are left behind the player as he navigates through the maze; letting him easily trace back to previous positions.  

Upon completion, regardless if the player managed to escape or not, the game renders the singular escape path to the player. If the player manages to escape the maze, he receives a completion score that is dependant on the time it took for him to reach the exit and the amount of mistakes he made such as hitting a wall or straying of the shortest path.
...
At the heart of the game is Wilsons Algorithm. Formally, it produces uniform spanning trees using loop-erased random walks. Perfect mazes are in essence spanning trees, but unlike other algorithms such as Prims, Kruskals and DFS, that tend to create predictable patterns or biases, Wilsons manages to generate truly random, perfect mazes.
...
The game has 4 difficulty levels: Easy, Medium, Hard and No Escape, with each one having different maze sizes and time limit (f.ex. Medium generates a $15 \times 15$ maze and gives the player 90 seconds to complete it).\
***points: 5***

#### 3.3 *Animated maze generation and shortest path*

We animated the maze generation in real time using ncurses, which shows how the Wilsons algorithm creates the maze. The shortest path is then animated at the end of each game, where the player can see how close their path was from the shortest path.\
***points: 10***
...
When displaying the maze and the player movement we had issues where the maze rendering would flicker since it was redrawing the whole maze for every update.
...
To solve this, we had to upscale every cell on rendering to accommodate for the scaling imbalance. This however wasn't enough, since the size of the horizontal- and vertical walls also differed, we needed to apply different scaling constants to them as well.
...
While the traditional approach makes use of adjecency lists/matrixes to store a graph's edges, we implicitly defined the edges by setting each directional wall of each `Cell` instance to `true` or `false` (has a wall in some direction and does not have a wall in some direction respectively).
...
Although, especially in retrospect, this proved to be more of a minor obstacle rather than a major one, it still required careful thought and consideration to implement correctly.

### 5. Estimated Total Points

#### *Achieved*

| Features Included               | Est. Points |
|---------------------------------|-------------|
| Wilson's algorithm (maze gen)   | *17 pts.*   |
| BFS Shortest path finder        | *13 pts.*   |
| ncurses real-time I/O           | *20 pts.*   |
| Animated maze generation        | *8 pts.*    |
| Animated path finder            | *8 pts.*    |
| Main menu                       | *10 pts.*   |
| Varied difficulties             | *5 pts.*    |
| Breadcrumb trail                | *5 pts.*    |
| Completion score system         | *5 pts.*    |
| Real-time keyboard input        | *5 pts.*    |
| **Total Points:**               | **96**      |
